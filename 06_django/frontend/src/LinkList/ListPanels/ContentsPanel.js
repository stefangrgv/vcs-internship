...
function ListContentsPanel (props) {
  ...
  useEffect ( () => {
    if (editId !== null) {
      context.showQuestionModal(
      <input ref={editInputRef} className='input-field input-field-large' defaultValue={editedURL} onChange={onChangeEditedURL} placeholder='Enter URL'/>,
      'Save', onclickSaveLink, 'Cancel', clearEditId);
    } else {
      context.hideModal();
    }
  }, [editId]);
